...
def get_ood_scores(loader, layer_num, in_dist=False):
    _score = []
    _right_score = []
    _wrong_score = []
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if layer_num < n_layers:
                lsgm_scores = -probs_test[layer_num].max(axis=1)
            else:
...
for layer_num in range(n_layers + 1):
    ...
    in_score, right_score, wrong_score = get_ood_scores(test_loader, layer_num, in_dist=True)
