...
def print_tnr95(out_score, in_score):
    scores = np.concatenate([out_score, in_score], axis=0)
    y_true = np.concatenate([np.ones_like(out_score), np.zeros_like(in_score)], axis=0)
    fpr, tpr, _ = sk.roc_curve(y_true, scores)
    # tnr
    tpr95 = np.where(tpr>=0.95)[0][0]
    tnr = 1 - fpr

    print('TNR95:\t\t\t{:.2f}'.format(100 * tnr[tpr95]))
